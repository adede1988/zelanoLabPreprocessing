...
%   commit 9791153, 2026-08-03).
    ...
    %   2. Windowed amplitude normalization on a 30-s moving STD scale (was
    %      60-s): adapts faster at loud->quiet transitions - fixes both the
    %      whole-minute dropouts of the un-windowed runs and the residual
    %      quiet-epoch misses of the 60-s version. NB a robust scale (movmad)
    %      was tried and REJECTED: for sparse sharp breathing on a flat
    %      baseline it tracks the noise floor (the peaks are outliers it
    %      ignores) and over-amplifies noise until detection collapses.
    Wn = round(30 * fs);
    sm = round(0.15 * fs);
    ...
    bmFeatures.conditioning = struct( ...
        ...
        'engineVersion',        'v3b (2026-08-26 QC review: 150 ms smoothing + 30 s movstd window + onset band correction)', ...
        'detectionSmoothing',   '150 ms moving mean before normalization', ...
        'windowedAmpNorm',      'detection trace = smoothed / max(movstd(smoothed, 30 s), floorFrac x median); raw data untouched', ...
        'ampNormWindowSec',     30, ...
