...
KG_JW_RSA_by_length.png    RSA by breath-length bin (medians only, no IQR bars), KG (orange) and JW (blue),
...
JW_RSA_surface.png         JW length x inhale-volume RSA surface (local-linear fit on good paced breaths),
                           inhale-volume axis limited to 50k-250k; grey = paced breaths, blue = ATB, diamond = ATB mean.
JW_ATB_error_surface.png   JW ATB observed-minus-predicted RSA over the same axes/conventions (diverging map,
                           ATB dots coloured by their own error, grey contours = the paced RSA surface).
