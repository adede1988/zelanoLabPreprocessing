% exportPack - export a compact analysis pack from a pacedBreathing preprocessed
% final so the respHRV summary/figures can be built off-server (the final itself is
% ~1.5 GB of EEG). READ-ONLY with respect to the final.
%
% Locating the final:
%   env ZLP_FINAL    full path to the preprocessed final .mat (preferred; keeps this
%                    script independent of the preprocessing repo)
%   otherwise, when run inside zelanoLabPreprocessing, applyParams + ZLP_PACK_ID
%   resolve it (env ZLP_PACK_ID session id, default 260915_EEG_NWU_KG)
%   env ZLP_PACK_OUT output folder (default <pwd>/pack)
%
...
%   bmObj, blocks, blockInference, segments, behDat (table), sessID, fs (500)
...
out = getenv('ZLP_PACK_OUT'); if isempty(out), out = fullfile(pwd, 'pack'); end
if ~isfolder(out), mkdir(out); end

% locate the preprocessed final: explicit path (ZLP_FINAL) keeps this independent of
% the preprocessing repo; otherwise fall back to applyParams when run inside it.
fpath = getenv('ZLP_FINAL');
if isempty(fpath)
    assert(exist('applyParams', 'file') == 2, ...
        'set ZLP_FINAL to the preprocessed final .mat (applyParams is not on the path)');
...
id = od.sessID;   % authoritative id from the final (correct even when ZLP_FINAL points elsewhere)
...
fprintf('exportPack: %s -> %s (%d breaths, %d blocks, %.1f min, hasRR=%d)\n', ...
